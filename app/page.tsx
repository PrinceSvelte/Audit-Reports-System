...
export default function AuditReportSystem() {
  ...
  const generatePDF = async (data: any) => {
    ...
    const drawTable = (
        ...
        if (multiLineHeaders) {
          ...
          lines.forEach((line: string, lineIndex: number) => {
            pdf.text(
              ...
              startTextY + lineIndex * lineHeight
            );
          });
        } else {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        row.forEach((cell, cellIndex) => {
          ...
          if (
            headers[cellIndex] === "Severity" &&
            severityColors[cell as keyof typeof severityColors]
          ) {
            const severityColor =
              severityColors[cell as keyof typeof severityColors];
      ...
      pdf.line(
        20,
        borderY,
        20 + columnWidths.reduce((a, b) => a + b, 0),
        borderY
      ); // Top border
    ...
    const addCoverPage = () => {
      ...
      pdf.setFontSize(16);
      pdf.setFont("helvetica", "normal");
      pdf.text("Comprehensive Security Assessment", 105, 70, {
        align: "center",
      });
      ...
      const auditDetailsText = [
        {
          label: "Release Date:",
          value: data.auditDetails?.reportReleaseDate || "Not specified",
        },
        {
          label: "Audit Type:",
          value: data.auditDetails?.typeOfAudit || "Not specified",
        },
        {
          label: "Report Type:",
          value: data.auditDetails?.typeOfAuditReport || "Not specified",
        },
        {
          label: "Period:",
          value: data.auditDetails?.period || "Not specified",
        },
      ];
      ...
      pdf.setFontSize(9);
      pdf.text(`Generated on ${new Date().toLocaleDateString()}`, 105, 260, {
        align: "center",
      });
    ...
    const addDocumentPreparation = () => {
      ...
      yPosition = addSectionHeader(
        data.headers.documentControl || "1. DOCUMENT PREPARATION",
        yPosition,
        colors.primary
      );

      const tableData =
        data.documentPreparation?.map((item: any) => [
          item.field,
          item.value,
        ]) || [];
    ...
    const addChangeHistory = () => {
      ...
      const tableData =
        data.changeHistory?.map((item: any) => [
          item.version,
          item.date,
          item.remarks,
        ]) || [];
    ...
    const addDistributionList = () => {
      ...
      const tableData =
        data.distributionList?.map((item: any) => [
          item.name,
          item.designation,
          item.email,
        ]) || [];
    ...
    const addEngagementScope = () => {
      ...
      checkNewPage(80);
      yPosition = addSectionHeader(
        data.headers.engagementScope || "5. ENGAGEMENT SCOPE",
        yPosition,
        colors.warning
      );
    ...
    const addAuditingTeam = () => {
      ...
      checkNewPage(60);
      yPosition = addSectionHeader(
        data.headers.auditingTeam || "6. AUDITING TEAM",
        yPosition,
        colors.primary
      );
    ...
    const addAuditActivities = () => {
      ...
      checkNewPage(60);
      yPosition = addSectionHeader(
        data.headers.auditActivities || "7. AUDIT ACTIVITIES",
        yPosition,
        colors.secondary
      );

      const tableData =
        data.auditActivities?.map((item: any) => [
          item.phase,
          item.description,
          item.timeline,
        ]) || [];
    ...
    const addToolsSoftware = () => {
      ...
      checkNewPage(60);
      yPosition = addSectionHeader(
        data.headers.toolsSoftware || "13. TOOLS & SOFTWARE",
        yPosition,
        colors.accent
      );

      const tableData =
        data.toolsSoftware?.map((item: any) => [
          item.sNo,
          item.name,
          item.version,
          item.license,
...
Some of the evidence found during the assessment are used in this report to support the findings and recommendations of this assessment.`;
      ...
      const criticalCount =
        data.vulnerabilities?.filter((v: any) => v.severity === "Critical")
          .length || 0;
      const highCount =
        data.vulnerabilities?.filter((v: any) => v.severity === "High")
          .length || 0;
      const mediumCount =
        data.vulnerabilities?.filter((v: any) => v.severity === "Medium")
          .length || 0;
      const lowCount =
        data.vulnerabilities?.filter((v: any) => v.severity === "Low").length ||
        0;
      ...
      pdf.setFontSize(14);
      pdf.setFont("helvetica", "bold");
      pdf.text("Risk Matrix", chartX + chartWidth / 2, chartY + 12, {
        align: "center",
      });
      ...
      chartData.forEach((item, index) => {
        ...
        pdf.setFontSize(9);
        pdf.setFont("helvetica", "bold");
        pdf.text(item.count.toString(), barX + barWidth / 2, barY - 2, {
          align: "center",
        });

        // Draw label below bar
        pdf.setFontSize(8);
        pdf.text(item.label, barX + barWidth / 2, chartStartY + 6, {
          align: "center",
        });
      });
      ...
      for (let i = 0; i <= maxScale; i += Math.ceil(maxScale / 5)) {
        ...
        if (i > 0) {
          ...
          pdf.line(
            chartStartX,
            y,
            chartStartX + 4 * (barWidth + barSpacing) - barSpacing,
            y
          );
        }
      }
    ...
    const addDetailedObservations = async () => {
      ...
      for (const obs of data.detailedObservations) {
        ...
        yPosition = addSectionHeader(
          `${
            data.headers.detailedObservation || "16. DETAILED OBSERVATION"
          } - ${index + 1}`,
          yPosition,
          colors.dark
        );
            ...
            if (poc.images && poc.images.length > 0) {
              for (let imgIndex = 0; imgIndex < poc.images.length; imgIndex++) {
                ...
                try {
                  ...
                  const imgData = await new Promise<string>(
                    (resolve, reject) => {
                  ...
                  const imageFormat = image.type.includes("png")
                    ? "PNG"
                    : "JPEG";
                  ...
                  pdf.text(
                    `[Image could not be loaded: ${image.name}]`,
                    25,
                    yPosition
                  );
                  yPosition += 10;
    ...
    if (data.visibleSections.introduction) {
      addContentSection(
        data.headers.introduction || "4. INTRODUCTION",
    ...
    if (data.visibleSections.preEngagement) {
      addContentSection(
        data.headers.preEngagement || "9. PRE-ENGAGEMENT",
        data.editableContent?.preEngagement || ""
      );
    }

    if (data.visibleSections.engagement) {
      addContentSection(
        data.headers.engagement || "10. ENGAGEMENT",
        data.editableContent?.engagement || ""
      );
    }
...
The SANS Top 25 is a list of the most critical software vulnerabilities and exposures that are actively being exploited. It is compiled by the SANS Institute, a leading cybersecurity training and research organization.`,
    ...
    preEngagement: `Discovery Meeting
...
• Documented checklist and prerequisites`,
    ...
    engagement: `a. Functional Walkthrough of Scope
...
• Vulnerabilities identified were as on the date testing conducted and also as per the scan policies (intrusive or non-intrusive) & plugins selected. There may be vulnerabilities which may exist and may not assess, since their exploits may lead to system downtime due to Denial of Service (DOS) attack. Also, the vulnerabilities identified after the scan date may also not form part of this report.`,
  ...
  const updateEditableContent = (
    key: keyof typeof editableContent,
    content: string
  ) => {
  ...
  const collectAllData = () => {
    return {
      ...
      documentPreparation:
        documentControlRef.current?.getDocumentPreparation() || [],
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const screenshotKeys = extractScreenshotKeys(
        exportData.detailedObservations || []
      );
      ...
      if (saveResult.success) {
        ...
        shadcnToast({
          ...
          description:
            "PDF created but failed to save to database. Download will proceed.",
          variant: "destructive",
        });
      }
      ...
      // Download PDF
      pdf.save(
        `security-audit-report-${new Date().toISOString().split("T")[0]}.pdf`
      );
    } catch (error) {
  ...
  const exportToWord = async () => {
    ...
    try {
      ...
      shadcnToast({
        ...
        description:
          "Your audit report has been exported as Word document successfully.",
      });
    } catch (error) {
      ...
      shadcnToast({
        title: "Export Failed",
        description:
          "There was an error generating the Word document. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsExporting(false);
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("documentControl", title)
                }
              />
              <Button
                variant="destructive"
            ...
            <CardContent>
              <DocumentControlTables
                ...
                onHeaderChange={(key, title) =>
                  updateEditableHeader(
                    key as keyof typeof editableHeaders,
                    title
                  )
                }
              />
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("tableOfContents", title)
                }
              />
              <Button
                variant="destructive"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("introduction", title)
                }
              />
              <Button
                variant="destructive"
            ...
            <CardContent>
              <EditableContentSection
                ...
                onContentChange={(content) =>
                  updateEditableContent("introduction", content)
                }
        ...
        {visibleTables.engagementScope && (
          <Card
            className={`${!reportID ? "pointer-events-none opacity-70" : ""}`}
          >
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                title={editableHeaders.engagementScope}
                onTitleChange={(title) =>
                  updateEditableHeader("engagementScope", title)
                }
              />
              {/* <Button
                variant="destructive"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("auditingTeam", title)
                }
              />
              <Button
                variant="destructive"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("auditActivities", title)
                }
              />
              <Button
                variant="destructive"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("auditMethodology", title)
                }
              />
              <Button
                variant="destructive"
            ...
            <CardContent className="space-y-6">
              <EditableContentSection
                ...
                onContentChange={(content) =>
                  updateEditableContent("auditMethodology", content)
                }
          ...
          <Card>
            ...
            <CardContent>
              <EditableContentSection
                ...
                onContentChange={(content) =>
                  updateEditableContent("preEngagement", content)
                }
              ...
              <EditableContentSection
                ...
                onContentChange={(content) =>
                  updateEditableContent("engagement", content)
                }
              ...
              <EditableContentSection
                title="3. Post-engagement"
                content={editableContent.postEngagement}
                onContentChange={(content) =>
                  updateEditableContent("postEngagement", content)
                }
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("engagement", title)
                }
              />
              <Button
                variant="destructive"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("postEngagement", title)
                }
              />
              <Button
                variant="destructive"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("riskMethodology", title)
                }
              />
              <Button
                variant="destructive"
            ...
            <CardContent>
              <EditableContentSection
                ...
                onContentChange={(content) =>
                  updateEditableContent("riskMethodology", content)
                }
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("toolsSoftware", title)
                }
              />
              <Button
                variant="destructive"
          ...
          >
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("vulnerability", title)
                }
              />
              {/* <Button
                variant="destructive"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <EditableHeader
                ...
                onTitleChange={(title) =>
                  updateEditableHeader("detailedObservation", title)
                }
              />
              <Button
                variant="destructive"
            ...
            <CardContent>
              <div className="space-y-6 pl-5">
                <ol className="list-decimal">
                  {bestPractices.map((bestPractice, index) => (
                    <li
                      key={index + "bestPractice"}
        ...
        {/* {visibleTables.webChecklist && (
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <h3 className="text-lg font-semibold">
                {editableHeaders.webChecklist}
              </h3>
            </CardHeader>
            <CardContent>
              <WebChecklistTable />
