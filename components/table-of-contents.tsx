...
export default function TableOfContents() {
  ...
  const [contents, setContents] = useState<TOCItem[]>([
    ...
    { title: "AUDIT METHODOLOGY AND CRITERIA / STANDARD REFERRED FOR AUDIT", page: 8 },
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex gap-2">
          {isEditing ? (
            <>
              <Button onClick={() => setIsEditing(false)} size="sm" variant="default">
            ...
            {isEditing ? (
              <>
                ...
                <Input
                  type="number"
                  value={item.page}
                  onChange={(e) => updateItem(index, "page", Number.parseInt(e.target.value) || 1)}
                  className="w-20 mr-2"
                />
              ...
              <>
                <span className="font-medium text-sm uppercase">{item.title}</span>
